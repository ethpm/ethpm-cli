...
def verify_contract(args: Namespace, config: Config) -> None:
    ...
    if runtime_bytecode == actual_bytecode:
        logger.info(
            "Valid: Contract code found at %s matches contract type: %s located in the %s package.",
            args.address,
            contract_type,
            package,
        )
        ...
        logger.info(
            "Invalid: Contract code found at %s does not match the contract type: "
            "%s located in the %s package.",
            args.address,
            contract_type,
            package,
        )
...
def get_contract_type_verification_data(
    package: str, contract_type: str, config: Config
) -> str:
    manifest = json.loads((config.ethpm_dir / package / "manifest.json").read_text())
    if contract_type not in manifest["contract_types"].keys():
        raise InstallError(
            f"{contract_type} not available in {package} package. Available contract "
            f"types include: {list(manifest['contract_types'].keys())}."
        )
